...
> 版本: V1.6.1 · 快捷命令: `volss` · 支持 Debian / Ubuntu / Alpine
...
```
  =================================================
    Shadowsocks-Rust 管理脚本    V1.6.1    快捷命令: volss
  =================================================
    安装: ● 已安装        服务: ● 运行中
    时间: 已同步 (±1s)
  -------------------------------------------------
...
| 功能 | 说明 |
|------|------|
| 查看用户列表 | 显示名称、端口、配额、到期日和正常/手动暂停/配额暂停/到期暂停状态 |
...
一次生成 Clash、Mihomo 和 sing-box 配置。Clash/Mihomo 的每个 SS 节点都明确包含 `udp: true`；sing-box 使用原生 Shadowsocks outbound。导出同时为每个 SS 链接生成 PNG 二维码，并在配置容量允许时生成 Clash 配置二维码。二维码可按用户直接显示在终端。
