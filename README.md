...
> 版本: V1.5.7 · 快捷命令: `volss` · 支持 Debian / Ubuntu / Alpine
...
```
  =================================================
    Shadowsocks-Rust 管理脚本    V1.5.7    快捷命令: volss
  =================================================
    安装: ● 已安装        服务: ● 运行中
    时间: 已同步 (±1s)
  -------------------------------------------------
...
新增用户时，脚本会自动沿用现有的加密方式，端口可选「自动顺延」（从当前最大端口往后找可用端口，跳过占用）或「手动指定」。新用户的密码随机生成，IPv4/IPv6 流量统计规则同步建立，原有用户的链接和流量不受影响。
...
IPv6 地址会按 URI 标准自动添加方括号，例如 `ss://...@[2001:db8::1]:30001#用户1`。
