...
| 项目 | 要求 |
|------|------|
| 系统 | Debian / Ubuntu（**不支持 OpenWrt / ImmortalWrt**） |
| 权限 | root |
| 架构 | x86_64 / aarch64 |
...
不支持。本脚本依赖 systemd、iptables-persistent、apt 等 Debian/Ubuntu 特有组件，OpenWrt 使用 procd 管理服务、fw4/nftables 管理防火墙、opkg 管理包，两者差异过大。OpenWrt 用户请直接执行：
