...
> 版本: V1.7.1 · 快捷命令: `volss` · 支持 Debian / Ubuntu / Alpine
...
- 单用户链接泄露处置：轮换密码并按 IPv4、IPv6 或双入口重新交付链接
...
```
  ...
  V1.7.1  |  2026-07-24 12:47:08  |  volss
...
| 重置单个用户密码/链接 | 密码泄露后仅轮换指定用户；可选择输出新版 IPv4、IPv6 或双入口链接 |
...
### 链接泄露处置（用户管理 > 密码与链接重置 > 1）

选择用户后可选择重新交付 IPv4、IPv6 或两个入口的新版链接。Shadowsocks 的双入口共用一个用户密码，因此为防止泄露，执行任意单用户重置都会同时更换该用户密码，**旧 IPv4 和 IPv6 链接都会失效**；选择项只决定终端本次显示和交付哪些新链接。用户名称、端口、入口地址、流量配额、到期时间和暂停状态不会改变。

重置后脚本会重启服务使新密码立即生效，并同步刷新 SS 链接、Clash/Mihomo/sing-box 导出和二维码。请只将终端显示的新链接发送给对应用户。
...
| 版本 | 说明 |
|------|------|
| V1.7.1 | 新增单用户密码与链接重置；可选择重新交付 IPv4、IPv6 或双入口链接，任意重置均同时废止该用户全部旧入口；同步刷新客户端导出和二维码 |
