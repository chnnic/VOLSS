...
> 版本: V1.6.2 · 快捷命令: `volss` · 支持 Debian / Ubuntu / Alpine
...
```
  =================================================
    Shadowsocks-Rust 管理脚本    V1.6.2    快捷命令: volss
  =================================================
    安装: ● 已安装        服务: ● 运行中
    时间: 已同步 (±1s)
  -------------------------------------------------
...
支持两种统计后端：原生 nftables `inet` 表同时覆盖 IPv4/IPv6，或 iptables + ip6tables 双栈链。已有安装会继续沿用当前后端；新安装优先使用 nftables，可通过选项 30 切换。流量数据持久化保存至 `traffic.json`，采用「读取增量 → 内核归零」机制确保统计准确，重启不丢失，直到手动重置才清零，并显示最后重置时间。
...
标准 `ss://` 链接没有声明 UDP 能力的字段，因此部分客户端只能显示 unknown，不能靠修改 SS 链接修复。选择菜单 24 导出的 Clash/Mihomo 配置会为每个节点明确写入 `udp: true`。服务端状态应以 `mode: tcp_and_udp`、UDP 监听或菜单 29 健康检查为准。
